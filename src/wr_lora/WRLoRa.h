...
/* ESP32-only fork: the upstream SAMD board matrix and LORA_DEFAULT_SPI
 * are gone with hardware SPI. Pins are always supplied by setPins() and
 * setSPIPins() before begin(). */
...
class WRLoRaClass : public Stream {
public:
  WRLoRaClass();
  ...
  void setSPIDelayNops(uint8_t nops);
  uint32_t spiBenchmarkHz();
